...
def main():
    ...
    test_output_dir = os.path.join(config.output_dir, 'anomaly_maps',
                                   'mvtec_ad', 'two_kinds', 'test')
    os.makedirs(train_output_dir)
    os.makedirs(test_output_dir)

    # load data
    train_set1 = ImageFolderWithoutTarget(
        os.path.join(dataset_path, 'screw', 'train'),
        transform=transforms.Lambda(train_transform))
    train_set2 = ImageFolderWithoutTarget(
        os.path.join(dataset_path, 'metal_nut', 'train'),
        transform=transforms.Lambda(train_transform))  
    full_train_set = ConcatDataset([train_set1, train_set2])          
    test_set1 = ImageFolderWithPath(
        os.path.join(dataset_path, 'screw', 'test'))
    test_set2 = ImageFolderWithPath(
        os.path.join(dataset_path, 'metal_nut', 'test'))  
    full_test_set = ConcatDataset([test_set1, test_set2])     
    ...
    for iteration, (image_st, image_ae), image_penalty in zip(
        ...
        if iteration % 10000 == 0 and iteration > 0:
            ...
            auc = test(
                test_set=full_test_set, teacher=teacher, student=student,
